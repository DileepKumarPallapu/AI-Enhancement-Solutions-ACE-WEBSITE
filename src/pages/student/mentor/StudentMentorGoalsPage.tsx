    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 font-sans py-10 px-4 sm:px-6 lg:px-8">
      <div className="max-w-4xl mx-auto space-y-8">
        ...
        <div>
          ...
          <h1 className="text-3xl font-extrabold text-white">Student Success Plan & Roadmap</h1>
        ...
        <div className="space-y-6">
          {studentGoals.map(goal => (
            <div key={goal.id} className="p-6 rounded-3xl bg-slate-900/80 border border-slate-800 space-y-5 shadow-xl">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                ...
                <div className="text-right">
                  <span className="text-xl font-black text-indigo-400 font-mono">{goal.progressPercentage}%</span>
                  <p className="text-[11px] text-slate-400">Target: {goal.targetDate}</p>
                </div>
              </div>
              ...
              <div className="space-y-2.5 pt-2">
                <h4 className="text-xs font-bold text-slate-300 uppercase tracking-wider">3-Month Milestone Roadmap</h4>
                {goal.milestones.map((milestone, idx) => (
                  <div
                    key={milestone.id}
                    onClick={() => handleToggleMilestone(goal.id, idx, milestone.completed)}
                    className="p-3.5 rounded-2xl bg-slate-950/60 border border-slate-800/80 flex items-start gap-3 cursor-pointer hover:border-slate-700 transition"
                  >
                    {milestone.completed ? (
                    ...
                    <div className="space-y-0.5">
                      <p className={`text-xs font-semibold ${milestone.completed ? 'line-through text-slate-500' : 'text-white'}`}>
                        Month {milestone.monthIndex}: {milestone.title}
                      </p>
                      <p className="text-[11px] text-slate-400 leading-relaxed">{milestone.description}</p>
                    </div>
                  </div>
                ))}
              </div>
              ...
              {goal.mentorReviewNotes && (
                <div className="p-3.5 rounded-2xl bg-indigo-950/30 border border-indigo-500/20 text-xs">
                  <span className="font-bold text-indigo-300">Mentor Guidance Feedback:</span>
                  <p className="text-slate-300 mt-1 italic">"{goal.mentorReviewNotes}"</p>
                </div>
              )}
            </div>
          ))}
