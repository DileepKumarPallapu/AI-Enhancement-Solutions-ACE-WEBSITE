  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 font-sans py-10 px-4 sm:px-6 lg:px-8">
      <div className="max-w-4xl mx-auto space-y-8">
        ...
        <div>
          <Link to="/student/mentor" className="inline-flex items-center gap-1.5 text-xs text-indigo-400 hover:underline mb-2">
            <ArrowLeft className="w-3.5 h-3.5" /> Back to Mentor Workspace
          </Link>
          <h1 className="text-3xl font-extrabold text-white">Mentorship Sessions & History</h1>
          <p className="text-xs text-slate-400 mt-1">Track scheduled calls, review summaries, and submit guidance feedback.</p>
        </div>

        {/* Sessions List */}
        <div className="space-y-4">
          {studentSessions.length === 0 ? (
            <p className="text-xs text-slate-500 py-6 text-center">No sessions recorded yet.</p>
          ) : (
              ...
              <div key={sess.id} className="p-6 rounded-3xl bg-slate-900/80 border border-slate-800 space-y-4 shadow-xl">
                <div className="flex items-start justify-between">
                  <div>
                    ...
                    <h3 className="text-base font-bold text-white mt-1.5">{sess.topic}</h3>
                    <p className="text-xs text-slate-400">with {sess.mentorName}</p>
                  </div>
                  <span className={`px-2.5 py-1 rounded-full text-xs font-bold ${
                    sess.status === 'CONFIRMED' ? 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/20' : 'bg-slate-800 text-slate-400'
                  }`}>
                    {sess.status}
                  </span>
                </div>
                ...
                {sess.mentorSummary && (
                  <div className="p-3.5 rounded-2xl bg-slate-950/60 border border-slate-800 text-xs space-y-1">
                    <span className="font-bold text-indigo-300">Mentor Summary:</span>
                    <p className="text-slate-300 leading-relaxed">{sess.mentorSummary}</p>
                  </div>
                )}
                ...
                {sess.studentFeedbackRating && (
                  <div className="text-xs text-amber-400 flex items-center gap-1">
                    ...
                    {sess.studentFeedbackComment && <span className="text-slate-400 italic">("{sess.studentFeedbackComment}")</span>}
                  </div>
                )}
              </div>
            ))
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-sm">
          <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 max-w-md w-full shadow-2xl space-y-4">
            ...
            <form onSubmit={handleFeedbackSubmit} className="space-y-4 text-xs">
              <div>
                <label className="block text-slate-300 font-semibold mb-2">Overall Guidance Rating</label>
                  ...
                  {[1, 2, 3, 4, 5].map(num => (
                    <button
                      ...
                      className={`w-10 h-10 rounded-xl font-bold transition ${
                        rating >= num ? 'bg-amber-500 text-slate-950' : 'bg-slate-800 text-slate-400'
                      }`}
                    >
                      {num}★
                    </button>
              ...
              <div>
                <label className="block text-slate-300 font-semibold mb-1">Constructive Feedback (Private)</label>
                <textarea
                  rows={3}
